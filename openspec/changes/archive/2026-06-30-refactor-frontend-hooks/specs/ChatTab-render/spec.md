...
#### Scenario: 渲染聊天消息列表
- **WHEN** ChatTab 渲染
- **THEN** 组件 MUST 通过 `useChatStore(s => s.chatMessages)` 获取消息列表，MUST NOT 用 props.messages；消息 ID 由 chatStore action 内部通过 `nextMsgId()` 生成
...
### Requirement: ChatTab 组件内 nextMsgId 必须迁移到共享工具
App.tsx 当前依赖 `nextMsgId()` 函数生成消息 ID（从 types.ts 导入）。迁移后，由于 `chatStore.send()` / `handleWsEvent()` 都需要追加 MUST 有一个全局唯一 ID。MUST 保留 `nextMsgId` 工具函数在 `electron/src/renderer/utils/nextMsgId.ts`（与 interpolate 同级）或 types.ts 中不动；chatStore MUST 从该导入引用，不得在 store 内自增计数器。

#### Scenario: nextMsgId 使用迁移
- **WHEN** chatStore action 追加 user/assistant/tool/system 任何角色消息
- **THEN** MUST 调 `nextMsgId()` 生成 ID；chatStore MUST 从 `../../utils/nextMsgId` 或 `../../types` 导入该函数
