...
- [ ] 1.1 在 `electron/package.json` 添加 `zustand` 依赖并执行 `npm install`
- [ ] 1.2 创建目录 `electron/src/renderer/stores/`、`electron/src/renderer/ws/`、`electron/src/renderer/utils/`
...
- [ ] 2.1 实现 `stores/connectionStore.ts`：持有 connected、wsUrl、connectionError、profiles、selectedProfile、connectMode、highlightMode（初始化从 localStorage('highlight-mode') 读取）、restartDialog、restarting（均作为 zustand state）；模块级 `let connectGen = 0` 和 `let connectedSnapshot = false`（不放进 state，供 handler 内部 mutation）；提供 connect（入口处 `connectGen++`，异步返回时检查过期）/ disconnect / restartConfirm / restartCancel / createProfile / handleBrowserDisconnect action（内部调对应 api）
- [ ] 2.2 在 `ws/gateway.ts` 接入 browser 事件分发：`chrome_disconnected` → `connectionStore.handleBrowserDisconnect()`
- [ ] 2.2b connectionStore.setHighlightMode MUST 内部自动同步 localStorage('highlight-mode', mode)；ConnectionBar 不再手动 setItem
- [ ] 2.3 App.tsx 删除所有浏览器连接相关的 useState/useRef，改为用 `useConnectionStore` selector 读取；保留 ConnectionBar JSX 但删除其 props，ConnectionBar 内部 subscribe store
- [ ] 2.4 `npm run build` 通过，手动验证：连接/断开/重启流程工作正常
...
- [ ] 3.5 `npm run build` 通过，手动验证：运行管道、查看日志、approve/reject review 工作正常
...
- [ ] 5.1 App.tsx 最终形态确认：只包含 TitleBar、ConnectionBar、TabBar、五个 Tab JSX（Exec/Chat/Log/Pipelines/Params/Settings）、StatusBar；无业务 state、无 useWebSocket/handleRun/handleConnect 等回调；render 所有 Tab（不通过 activeTab 条件渲染）
- [ ] 5.2 ChatTab 删除所有 props（改为内部 chatStore + connectionStore + pipelineStore selector）；treeNodes 在 ChatTab 内部通过 `useMemo` 从 chatStore + pipelineStore 数据组装
- [ ] 5.3 ParamsTab 删除 props，改为内部 `useCredentialStore` selector
- [ ] 5.4 SettingsTab 删除 props，改为内部 `useConnectionStore` + `useUiStore` selector
- [ ] 5.5 `npm run build` 通过，检查 App.tsx 行数 < 100
