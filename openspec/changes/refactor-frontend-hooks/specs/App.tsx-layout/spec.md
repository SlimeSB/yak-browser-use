...
### Requirement: App.tsx 必须瘦身为布局编排器
App.tsx MUST 不再持有任何业务 state（所有 useState 必须迁移到对应 store），MUST 只保留 JSX 结构：TitleBar、ConnectionBar、TabBar、TabContent、StatusBar。

#### Scenario: App 渲染
- **WHEN** 首次渲染
- **THEN** App MUST 渲染各 Tab 组件且 MUST 不传递任何业务 props（activeTab 由 uiStore 内部订阅）

#### Scenario: 切换 Tab
- **WHEN** 用户点击 ChatTab
- **THEN** uiStore.setActiveTab('agentmd') MUST 被调用，App 渲染 ChatTab，ChatTab 内部 MUST subscribe chatStore/connectionStore

### Requirement: App.tsx 不再内联 WebSocket handler
