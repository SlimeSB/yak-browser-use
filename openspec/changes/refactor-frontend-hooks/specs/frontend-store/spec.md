...
- `handleRunEnd` — 处理运行结束语义（重置 loading / currentRunId / currentPipeline）
...
- **THEN** chatStore MUST 调 `handleWsEvent(event)`，内部：
  1. 如果 editId 未处理过：加入 processedEditIds，追加到 pendingEdits
  2. 如果 editId 已处理过：更新现有 edit.modified 和 explanation
  3. 调 `api.listPipelines()` 刷新 pipelines 缓存（结果 MUST 通过 pipelineStore.refreshPipelines 或内部同步写入 pipelineStore 状态）
