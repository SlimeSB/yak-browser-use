...
系统 MUST 支持向浏览器页面注入交互元素的高亮编号标签。编号使用 `@e1, @e2, ...` 自增格式，与 `simplify-dom.js` 的 `@eN` 体系一致。高亮覆盖层使用 `position: absolute` 定位，JS 渲染时通过 `rect.left + window.scrollX` / `rect.top + window.scrollY` 转换坐标。
...
系统 MUST 支持移除页面上的所有高亮编号标签，同时清空 Python 侧的映射缓存。
...
系统 MUST 支持通过 `@eN` 编号从 Python 侧缓存中查询元素信息，不走 DOM 查询。输入 ref 支持 `"@e3"`、`"e3"`、`"3"` 格式，内部归一化为 `"@eN"` 后查询。
...
系统 MUST 在 goto/click/fill 操作成功后自动刷新高亮编号，保持编号与页面 DOM 同步。snapshot/scroll/source/eval 操作不触发刷新。
...
#### Scenario: scroll 后不刷新
- **WHEN** 执行 `browser_scroll` 成功
- **THEN** 不触发高亮刷新
...
系统 MUST 支持 chat 模式下 Agent 直接使用 `@eN` 作为 click/fill 的 selector。当 `element_map` 不可用时，通过 `cdp_helpers.get_element_by_index()` 做 fallback 查询。
