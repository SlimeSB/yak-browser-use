...
`api_run` endpoint MUST 在首次 run_pipeline 返回 failure_context 时进入 recovery 循环，循环次数不超过 MAX_RECOVERY_ATTEMPTS（默认 3 次）。每次循环新建独立 agent session，发送 recovery prompt，等待 agent 修复 yaml 后重跑 pipeline。agent 接管后通过 conversation_loop 自主完成剩余 pipeline 步骤。re-run 时从 `WORKSPACES_ROOT / pipeline_name / "pipeline.yaml"` 重新读取并 parse（唯一真相源，agent 通过 edit_pipeline 修改后的文件）。
