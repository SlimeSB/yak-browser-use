...
系统 MUST 提供一个 `skill_list` 工具，允许 Agent 列出所有可用 skill 的名称、描述和标签。tool handler MUST 返回 `{"ok": True, "result": "<JSON 字符串>"}`，`result` 是 `json.dumps(result, ensure_ascii=False)` 序列化的数组。

#### Scenario: 列出所有 skill
- **WHEN** Agent 调用 `skill_list()`
- **THEN** 系统返回 `{"ok": true, "result": "<JSON 字符串>"}`
- **AND** JSON 数组中每个元素包含 `name`、`description`、`tags` 字段
- **AND** 优先扫描子目录格式（`prompts/skill/<name>/SKILL.md`），其次扫描平面文件（`prompts/skill/<name>.md`）
