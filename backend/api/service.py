...
class Service:
    ...
    def _migrate_chat_session_if_needed(self, pipeline_name: str) -> None:
        """Migrate __chat__ session to a new pipeline workspace.

        Called when a pipeline.yaml is created from __chat__ mode.
        """
        import shutil

        if self._active_pipeline != _DEFAULT_PIPELINE:
            return
        if pipeline_name == _DEFAULT_PIPELINE:
            return

        src = _WORKSPACES_DIR / _DEFAULT_PIPELINE / "session"
        dst = _WORKSPACES_DIR / pipeline_name / "session"

        if not src.exists():
            return
        if dst.exists():
            logger.warning("Session dir already exists at %s, skipping migration", dst)
            return

        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(src), str(dst))
        logger.info("Migrated session from %s to %s", src, dst)

        # Update in-memory session key
        session = self._sessions.get(_DEFAULT_PIPELINE)
        if session:
            session.pipeline_name = pipeline_name
            self._sessions[pipeline_name] = session
            del self._sessions[_DEFAULT_PIPELINE]
    ...
    def _push_auto_record_review(self, pipeline_name: str, pipeline_text: str, session_id: str) -> None:
        ...
        # Check if this is a __chat__ → new pipeline migration
        if self._active_pipeline == _DEFAULT_PIPELINE:
            self._migrate_chat_session_if_needed(pipeline_name)
