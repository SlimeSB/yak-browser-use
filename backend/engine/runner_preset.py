...
def _extract_code_from_response(completion: str) -> str:
    ...
    Looks for a markdown code block (`` ```python ... ``` ``) and returns
    its content. Falls back to the entire *completion* string if no block
    is found.
    """
...
def _check_safe_imports(code: str) -> str | None:
    """AST-scan *code* for dangerous module imports and builtin calls.

    Returns ``None`` if the code is safe, or a rejection reason string
    (e.g. ``"禁止导入危险模块: os"``) if a dangerous import or call is detected.
    ...
    lines = ["async def _dummy():"] + ["    " + line for line in dedented.split("\n")]
    wrapped = "\n".join(lines)
    try:
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                if top in DANGEROUS_MODULES:
                    return f"禁止导入危险模块: {top}"
        elif isinstance(node, ast.ImportFrom):
            if node.level and node.level > 0:
                return "禁止使用相对导入 (from . import ...)"
            if node.module:
...
def _build_tool_shell(func_name: str, code_body: str) -> str:
    """Wrap generated code body in a standard tool file shell.

    The shell provides the async function signature, imports, and the
    ToolContext type annotation so the file is self-contained.
    """
    return f'''"""Auto-generated tool: {func_name} — inline generated by pipeline runner."""
...
def _tool_name_to_func_name(tool_name: str) -> str:
    """Convert a tool name to a valid Python function name.

    Strips ``_PH-`` prefix and replaces hyphens with underscores.
    ``_PH-crack-captcha`` → ``crack_captcha``.
    """
    from engine._lifecycle.tool_runner import _PH_PREFIX
    name = tool_name[len(_PH_PREFIX):] if tool_name.startswith(_PH_PREFIX) else tool_name
    return name.replace("-", "_")


async def _inline_generate_and_execute(
    *,
    tool_name: str,
    pipeline_name: str,
    tools_dir: Path,
    ...
    """Generate a _PH- tool inline, write it to disk, execute it, and rename.

    Flow:
    1. Capture page state (simplified snapshot + current URL)
    2. Build prompt → call LLM → extract code
    3. AST safety check → py_compile syntax check
    4. Inject shell → write to ``tools_dir/_PH-{name}.py``
    5. Create ToolContext → execute the function
    6. On success: rename file + register schema + update YAML refs
    7. On failure: retry up to 3 times with error feedback
    """
    from engine.ops import ToolContext
    from engine._lifecycle.tool_runner import ToolRunner

    runner = ToolRunner(tools_dir, pipeline_name)
    func_name = _tool_name_to_func_name(tool_name)

    tools_dir.mkdir(parents=True, exist_ok=True)

    if prompt_template_path is None:
        prompt_template_path = Path(__file__).resolve().parent.parent / "prompts" / "tool_gen" / "generate.md"
    try:
        ...
        return {
            ...
            "error": {
                "code": "LLM_ERROR",
                "message": f"Prompt template not found: {prompt_template_path}",
            },
        }
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            logger.warning("inline_gen: snapshot capture failed: %s", e)
        try:
        ...
        page_state = f"URL: {current_url}\n\n页面摘要:\n{simplified_html}" if simplified_html else f"URL: {current_url}"
        ...
        prompt = prompt.replace("{page_state}", page_state)
        prompt = prompt.replace("{task_description}", f"工具名: {tool_name}")
        ...
        try:
            ...
            logger.warning("inline_gen: LLM call failed (attempt %d): %s", attempt, e)
            last_error = f"LLM调用失败: {e}"
            continue

        completion = getattr(response, "completion", "") or getattr(response, "content", "")
        code_body = _extract_code_from_response(completion)

        if not code_body:
            last_error = "LLM未返回有效代码"
            logger.warning("inline_gen: no code extracted from LLM response (attempt %d)", attempt)
            continue

        safety_result = _check_safe_imports(code_body)
        if safety_result is not None:
            last_error = safety_result
            logger.warning("inline_gen: safety check failed (attempt %d): %s", attempt, safety_result)
            continue

        try:
            full_code = _build_tool_shell(func_name, code_body)
            compile(full_code, "<inline_gen>", "exec")
        except SyntaxError as e:
            last_error = f"语法错误: {e}"
            logger.warning("inline_gen: compile check failed (attempt %d): %s", attempt, e)
            continue

        ph_path = tools_dir / f"{tool_name}.py"
        ph_path.write_text(full_code, encoding="utf-8")
        logger.info("inline_gen: wrote %s (attempt %d)", ph_path, attempt)
        ...
        try:
            ...
            logger.warning("inline_gen: execution failed (attempt %d): %s", attempt, e)
            continue

        if not exec_result.get("ok"):
            last_error = exec_result.get("error", "未知执行错误")
            logger.warning("inline_gen: tool returned error (attempt %d): %s", attempt, last_error)
            continue

        rename_result = runner.rename_ph_file(tool_name)
        if not rename_result.get("ok"):
            last_error = f"Rename失败: {rename_result.get('error', '')}"
            logger.warning("inline_gen: rename failed (attempt %d): %s", attempt, last_error)
            continue

        if pipeline_path is not None:
            refs_result = runner.update_pipeline_refs(tool_name, _tool_name_to_func_name(tool_name), pipeline_path)
            if not refs_result.get("ok"):
                last_error = f"Pipeline YAML更新失败: {refs_result.get('error', '')}"
                logger.warning("inline_gen: update_pipeline_refs failed (attempt %d): %s", attempt, last_error)
                continue

        logger.info("inline_gen: %s generated and renamed successfully (attempt %d)", tool_name, attempt)
        return {
            "status": "completed",
            "upgraded": True,
    ...
    return {
        ...
        "error": {
            "code": "LLM_ERROR",
            "message": f"Tool '{tool_name}' generation failed after {max_retries} attempts. Last error: {last_error}",
        },
    }
...
async def _execute_tool_step_with_guardian(
    ...
    If the tool name starts with ``_PH-`` and the file does not exist,
    triggers inline generation via ``_inline_generate_and_execute``.
    """
                ...
                return {
                    ...
                    "error": {
                        ...
                        "message": (
                            ...
                            f"and no llm_call is available for inline generation."
                        ),
                    },
                }
            return await _inline_generate_and_execute(
                tool_name=tool_name,
        ...
        if rename_result.get("ok"):
            if pipeline_path is not None:
                refs_result = runner.update_pipeline_refs(
                    tool_name, _tool_name_to_func_name(tool_name), pipeline_path,
                )
                if not refs_result.get("ok"):
                    logger.warning("update_pipeline_refs failed: %s", refs_result.get("error"))
