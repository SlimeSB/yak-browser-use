...
| 6 | **不 spawn 子 Agent** — `goal_run` 是主 LLM 的模式切换（todo + browser_* 工具），不是嵌套的 browser-use Agent 启动。整个对话保持在一个上下文窗口里、一套工具集下。 | 消除了多 Agent 架构的隔离开销、上下文碎片化和同步 bug。更简单、更快、更可预测。 |
